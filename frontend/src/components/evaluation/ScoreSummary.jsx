...
export default function ScoreSummary({ scores, onSubmit, submitting }) {
  ...
  // Theme colors matching your refined system
  const color = weighted >= 80 ? '#00D4AA' : weighted >= 60 ? '#F5A623' : '#E05260'
    ...
    <div className="relative overflow-hidden rounded-[24px] border border-white/5 bg-bg-2/60 backdrop-blur-xl p-8 shadow-2xl transition-all duration-500">
      ...
      <div className="relative z-10 flex flex-col items-center">
        <header className="mb-6">
          ...
          <h3 className="text-white text-lg font-bold tracking-tight leading-none">Weighted Total</h3>
        </header>
        ...
        <div className="w-full space-y-3 mb-8">
          ...
          <div className="space-y-1.5">
            {RUBRIC.map(r => {
              const contribution = ((scores[r.id] ?? 0) * r.weight / 100).toFixed(1);
              return (
                <div 
                  key={r.id} 
                ...
                >
                  <div className="flex flex-col items-start">
                    ...
                    <span className="text-[9px] text-text-3 font-medium opacity-60">Weight: {r.weight}%</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] font-mono text-text-3">{scores[r.id] ?? 0}/100</span>
                    <div className="w-px h-3 bg-white/10" />
                    <span 
                      className="text-xs font-bold font-mono"
                      style={{ color: weighted > 0 ? color : 'var(--text-3)' }}
                    >
                      +{contribution}
                    </span>
                  </div>
        ...
        <div className="w-full pt-6 border-t border-white/5 space-y-5">
           <div className="flex justify-between items-center px-1">
              <span className="text-[10px] text-text-3 font-bold uppercase tracking-widest">Confidence Index</span>
          ...
          <Button
            onClick={onSubmit}
            disabled={submitting}
            className={`w-full py-6 font-bold text-sm shadow-xl transition-all active:scale-[0.98]
              ${weighted === 0 ? 'opacity-50 pointer-events-none' : ''}
            `}
            style={{ 
