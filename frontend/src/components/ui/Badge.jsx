...
export default function Badge({ type = 'info', children, className = '' }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 text-[9px] font-bold uppercase px-2 py-0.5 rounded-full border ${styleConfig.replace('has-dot', '')} ${className}`}
    >
      {hasDot && <span className="w-1 h-1 rounded-full bg-current animate-pulse" />}
      {children}
    </span>
  )
}
