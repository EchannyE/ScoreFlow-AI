...
const CHART = [
  { d: 'Apr 1', s: 89, sc: 52 }, { d: 'Apr 2', s: 118, sc: 71 },
  { d: 'Apr 3', s: 142, sc: 96 }, { d: 'Apr 4', s: 162, sc: 134 },
]
...
export default function AdminDashboard() {
  ...
  const submissionsByCampaign = useMemo(() => submissions.reduce((grouped, submission) => {
  ...
  const scored  = submissions.filter(s => s.finalScore != null).length
  ...
  const handleAssign = async submission => {
    const evaluatorId = assignmentDrafts[submission._id] ?? submission.assignedEvaluatorId?._id ?? ''
    if (!evaluatorId) {
    ...
    try {
      ...
      setAssignmentError(error.response?.data?.message ?? error.message)
    } finally {
      setAssigningId('')
    }
  }
  ...
  return (
    <div className="fade-up space-y-8">
      ...
      <div className="flex flex-col gap-1">
        ...
        <h2 className="font-display font-bold text-3xl tracking-tight text-white">Dashboard</h2>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Scored"             value={scored}             color="#7B5EA7" icon="⚖" />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.6fr_1fr] gap-6">
        <Card className="flex flex-col">
          <div className="flex justify-between items-center mb-8">
            <span className="text-[10px] text-text-3 font-bold uppercase tracking-widest">Growth Velocity</span>
            <div className="flex gap-4 text-[10px] font-bold uppercase tracking-tighter">
              <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-green" /> Submissions</span>
            </div>
          </div>
        ...
        <Card className="flex flex-col">
          <div className="flex justify-between items-center mb-8">
            <span className="text-[10px] text-text-3 font-bold uppercase tracking-widest">Campaign Progress</span>
            <Link to="/admin/campaigns" className="text-[10px] text-green font-bold uppercase tracking-widest hover:underline">View All</Link>
          </div>
              ...
              <div key={c._id} className="group cursor-default">
                <div className="flex justify-between items-center mb-2.5">
                  <div className="flex flex-col">
                    <span className="text-xs font-bold text-white tracking-tight group-hover:text-green transition-colors line-clamp-1">{c.title}</span>
                  </div>
                  <Badge type={c.status} className="scale-75 origin-right">{c.status}</Badge>
                </div>
                <ProgressBar pct={c.completionPct ?? 0} color={c.status === 'active' ? '#00D4AA' : '#334155'} height={5} />
              </div>
            ))}
          </div>
        </Card>
      ...
      <div className="space-y-5">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div>
            <div className="text-[10px] text-text-3 font-bold uppercase tracking-[0.2em] mb-1">Campaign Assignment Board</div>
            <h3 className="font-display font-bold text-2xl tracking-tight text-white">Entries grouped by campaign</h3>
          </div>
          <Link to="/admin/submissions" className="text-[10px] text-green font-bold uppercase tracking-widest hover:underline px-3 py-1 bg-green/5 rounded-full border border-green/10 w-fit">
            Full Audit Log →
          </Link>
        </div>
        ...
        <div className="space-y-5">
          {campaigns.map(campaign => {
            ...
            const assignedCount = entries.filter(entry => entry.assignedEvaluatorId?._id).length
            const scoredCount = entries.filter(entry => entry.status === 'scored').length
              ...
              <Card key={campaign._id} className="overflow-hidden !p-0 border-white/5 shadow-2xl">
                  ...
                  <div className="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-4">
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        <h4 className="font-display font-bold text-2xl tracking-tight text-white">{campaign.title}</h4>
                    ...
                    <div className="grid grid-cols-3 gap-3 min-w-[280px]">
                      <div className="rounded-2xl border border-white/5 bg-bg-1/60 px-4 py-3">
                        <div className="text-[9px] text-text-3 font-bold uppercase tracking-widest mb-1">Entries</div>
                        <div className="text-xl font-mono font-bold text-white">{entries.length}</div>
                      </div>
                      <div className="rounded-2xl border border-white/5 bg-bg-1/60 px-4 py-3">
                        <div className="text-[9px] text-text-3 font-bold uppercase tracking-widest mb-1">Assigned</div>
                        <div className="text-xl font-mono font-bold text-white">{assignedCount}</div>
                      </div>
                      <div className="rounded-2xl border border-white/5 bg-bg-1/60 px-4 py-3">
                        <div className="text-[9px] text-text-3 font-bold uppercase tracking-widest mb-1">Scored</div>
                        <div className="text-xl font-mono font-bold text-white">{scoredCount}</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {entries.length === 0 ? (
                  ...
                  <div className="divide-y divide-white/5">
                    {entries.map(entry => {
                      const selectedEvaluatorId = assignmentDrafts[entry._id] ?? entry.assignedEvaluatorId?._id ?? ''

                      return (
                        <div key={entry._id} className="px-6 py-5 flex flex-col 2xl:flex-row 2xl:items-center gap-5">
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-center gap-2 mb-2">
                              <div className="text-sm font-bold text-white truncate">{entry.title}</div>
                              <Badge type={entry.status}>{entry.status?.replace('_', ' ')}</Badge>
                          ...
                          <div className="flex flex-col md:flex-row md:items-center gap-3 2xl:min-w-[460px]">
                            ...
                            >
                              ...
                              {evaluators.map(evaluator => (
                                <option key={evaluator._id} value={evaluator._id}>
                                  {evaluator.name}{evaluator.expertise?.length ? ` • ${evaluator.expertise.join(', ')}` : ''}
                                </option>
                              ))}
                            </select>
                            ...
                            <Button
                              ...
                              disabled={entry.status === 'scored' || !selectedEvaluatorId || assigningId === entry._id}
                              className="md:min-w-[150px]"
                            >
                              {assigningId === entry._id ? 'Assigning...' : entry.assignedEvaluatorId?._id ? 'Reassign Entry' : 'Assign Entry'}
                            </Button>
                          </div>

                          <div className="text-[11px] text-text-3 2xl:min-w-[180px]">
                            <div className="uppercase tracking-widest text-[9px] font-bold mb-1">Assigned evaluator</div>
