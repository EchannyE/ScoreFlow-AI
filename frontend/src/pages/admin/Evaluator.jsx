...
export default function AdminEvaluators() {
  ...
  const evaluatorCards = useMemo(() => {
    return evaluators.map(evaluator => {
      ...
      const completed = assignedSubmissions.filter(
        submission => submission.status === 'completed'
      ).length

      const pending = assignedSubmissions.filter(
        submission => submission.status !== 'completed'
      ).length
    ...
    <div className="fade-up space-y-8">
      ...
      <div className="space-y-3">
        ...
        {evaluatorCards.map(e => {
          const bCol =
            e.pct === 100 ? '#00D4AA' : e.pct > 50 ? '#F5A623' : '#E05260'
              ...
              <div className="grid grid-cols-1 xl:grid-cols-[1.4fr_0.9fr_0.9fr_0.9fr_1.1fr] items-center">
                <div className="p-5 flex items-center gap-4 border-r border-white/5">
                  <div className="relative">
                    <div
                      className="w-11 h-11 rounded-xl bg-gradient-to-br from-purple/20 to-blue/20 
                                  border border-white/10 flex items-center justify-center 
                                  text-xs font-bold text-white flex-shrink-0 shadow-inner"
                    >
                      {e.name?.split(' ').map(w => w[0]).join('').slice(0, 2) || 'EV'}
                    </div>

                    <div
                      className={`absolute -bottom-1 -right-1 w-3 h-3 rounded-full border-2 border-bg-2 
                                    ${e.isActive ? 'bg-green' : 'bg-orange'}`}
                    />
                  </div>

                  <div className="min-w-0">
                    <div className="text-sm font-bold text-white truncate group-hover:text-green transition-colors">
                      {e.name}
                    </div>
                    <div className="text-[11px] text-text-3 truncate mt-1">
                      {e.email}
                    </div>
                    ...
                    <div className="flex gap-1 mt-1.5 overflow-hidden flex-wrap">
                      {(e.expertise?.length ? e.expertise : ['General']).map(x => (
                        <span
                          key={x}
                          className="text-[8px] font-bold uppercase tracking-tighter text-text-3 bg-white/5 px-1.5 py-0.5 rounded-md border border-white/5"
                        >
                          {x}
                        </span>
                      ))}
                ...
                <div className="p-5 text-center border-r border-white/5">
                  ...
                  <div
                    className="font-mono font-bold text-lg text-green"
                    style={{ textShadow: '0 0 10px #00D4AA33' }}
                  >
                    {e.completed}
                  </div>
                </div>
                ...
                <div className="p-5 text-center border-r border-white/5 bg-white/[0.01]">
                  ...
                  <div className="text-sm font-mono font-bold text-text-2">
                    {e.pending}
                  </div>
                </div>

                <div className="p-5 flex flex-col gap-3 items-end bg-gradient-to-l from-white/[0.02] to-transparent">
                  <Badge type={e.statusType} className="scale-90 origin-right">
                    {e.statusLabel}
                  </Badge>
