...
const FILTERS = ['all', 'submitted', 'under_review', 'scored', 'flagged']
...
export default function AdminSubmissions() {
  ...
  if (loading || evaluatorsLoading) return (
  ...
  const list =
    ...
    : filter === 'flagged' ? submissions.filter(s => s.flagged)
    ...
    <div className="fade-up space-y-8">
      ...
      <div className="flex justify-between items-end border-b border-white/5 pb-8">
        <div>
          ...
          <h2 className="font-display font-bold text-3xl tracking-tight text-white">Submissions</h2>
        </div>
        <div className="text-right">
          <span className="text-[10px] text-text-3 font-bold uppercase tracking-widest block mb-1">Total Entries</span>
      ...
      <div className="flex gap-2 p-1.5 bg-bg-1 border border-white/5 rounded-2xl w-fit">
        {FILTERS.map(f => (
          <button 
            ...
            onClick={() => setFilter(f)}
            className={
              `text-[10px] px-5 py-2 rounded-xl cursor-pointer font-bold
      ...
      <div className="flex flex-col gap-3">
        {list.length === 0 ? (
          <div className="py-20 text-center border-2 border-dashed border-white/5 rounded-2xl opacity-40">
            <span className="text-xs font-bold uppercase tracking-widest text-text-3">No matching entries found</span>
          </div>
        ) : (
          list.map(s => {
            const selectedEvaluatorId = assignmentDrafts[s._id] ?? s.assignedEvaluatorId?._id ?? ''
            ...
            <Card 
              ...
              className={`!p-4 group relative ${s.flagged ? 'border-red/40 bg-red/5' : ''}`}
            >
              {/* Flagged Visual Indicator */}
              {s.flagged && (
                <div className="absolute top-0 left-0 bottom-0 w-1 bg-red shadow-[4px_0_12px_rgba(224,82,96,0.3)]" />
              )}

              <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:gap-6">
                {/* AI suggested score ring - Using purple for Neural theme */}
                <div className="flex-shrink-0 relative">
                   <div className="absolute inset-0 blur-lg bg-purple/10 rounded-full" />
                   <ScoreRing score={s.ai?.suggestedScore ?? 0} size={52} color="#A855F7" strokeWidth={5} />
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-[10px] text-text-3 font-medium uppercase tracking-wider">
                    <span className="text-text-2 font-bold">{s.submitter?.name ?? 'Anonymous'}</span>
                    <span className="opacity-30">•</span>
                    <span>{s.track}</span>
                    <span className="opacity-30">•</span>
                    <span className="font-mono">{new Date(s.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</span>
                  </div>
                  <div className="mt-2 text-[10px] text-text-3 font-medium uppercase tracking-wider">
                ...
                <div className="flex items-center gap-4 flex-shrink-0 xl:ml-auto">
                  <div className="flex flex-col items-end gap-1.5">
                    <div className="flex gap-2">
                      {s.flagged && <Badge type="flagged">Violation Flag</Badge>}
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <Input
                    ...
                    disabled={s.status === 'scored' || evaluators.length === 0}
                ...
                <div className="flex flex-col gap-2 xl:items-end xl:min-w-[220px]">
                  <Button
                    ...
                    disabled={s.status === 'scored' || !selectedEvaluatorId || assigningId === s._id || evaluators.length === 0}
                    className="w-full xl:w-auto"
                  >
                    {assigningId === s._id ? 'Assigning...' : s.assignedEvaluatorId?._id ? 'Reassign Project' : 'Assign Project'}
                  </Button>
                  <button
                    type="button"
                    onClick={() => navigate(`/admin/submissions/${s._id}`)}
