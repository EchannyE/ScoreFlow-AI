            ...
            <div className="flex flex-col md:flex-row items-start md:items-center gap-6 relative z-10">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  {s.flagged && <Badge type="flagged" className="scale-90">Attention Required</Badge>}
                </div>
                
                <p className="text-xs text-text-2 leading-relaxed mb-4 line-clamp-2 italic opacity-80">
                  {s.ai?.summary || 'Entry data is being synchronized for evaluation...'}
                </p>

                <div className="flex items-center gap-4">
                  <div className="flex flex-col">
                    <span className="text-[9px] text-text-3 font-bold uppercase tracking-widest">Vertical</span>
                    <span className="text-[11px] text-text-1 font-medium">{s.track}</span>
                  </div>
                  <div className="w-px h-6 bg-white/5" />
                  <div className="flex flex-col">
                    <span className="text-[9px] text-text-3 font-bold uppercase tracking-widest">Entry Date</span>
                    <span className="text-[11px] text-text-1 font-mono">{new Date(s.createdAt).toLocaleDateString()}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex md:flex-col items-center md:items-end justify-between w-full md:w-auto gap-4 pt-4 md:pt-0 border-t md:border-t-0 border-white/5">
                ...
                <Link to={`/evaluator/score/${s._id}`} className="w-full md:w-auto">
                  <Button className="w-full md:w-32 py-3.5 shadow-lg shadow-green/10 hover:shadow-green/20">
                    Grade Entry
                  </Button>
                </Link>
              </div>
