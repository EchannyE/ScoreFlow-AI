...
const DEFAULT_SCORES = { innovation: 70, feasibility: 70, impact: 70, presentation: 70 }
...
export default function EvaluatorScore() {
  ...
  const weightedScore = useMemo(
    () => Math.round(RUBRIC.reduce((acc, rubricItem) => (
  ...
  useEffect(() => {
    submissionsAPI.get(id)
      .then(r => setSubmission(r.data.data ?? r.data))
  ...
  const handleSubmit = async () => {
    try {
      setSubmitting(true)
      const evaluation = await submitScore({ submissionId: id, campaignId: submission.campaignId, scores, note })
  ...
  if (!submission) return <div className="text-text-3 text-center pt-16">Not found</div>
      ...
      <div className="grid grid-cols-[1fr_320px] gap-5">
        <div>
          <Card className="mb-4">
            ...
            <h3 className="font-display font-bold text-xl mb-2">{submission.title}</h3>
            <p className="text-xs text-text-2 leading-7 mb-3">{submission.ai?.summary}</p>
          ...
          <Card>
            ...
            <RubricSliders
              scores={scores}
              onChange={(key, val) => setScores(prev => ({ ...prev, [key]: val }))}
            />
            <Input
              as="textarea" label="Evaluator Notes"
        ...
        <div className="flex flex-col gap-3.5">
          <ScoreSummary scores={scores} onSubmit={handleSubmit} submitting={submitting} />
            ...
            <div className="flex items-start justify-between gap-4 border-b border-white/5 px-6 py-5 md:px-8">
              <div>
                ...
                <p className="mt-2 text-sm text-text-3 max-w-2xl">
                  Your evaluation is now locked in. Review how your final weighted score compares with the AI suggestion before returning to the queue.
                </p>
              </div>
            ...
            <div className="grid gap-6 px-6 py-6 md:grid-cols-[1.1fr_0.9fr] md:px-8 md:py-8">
                ...
                <div className="rounded-[22px] border border-white/5 bg-bg-2/60 p-5">
                  <div className="mb-4 flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-lg font-bold text-white">{submission.title}</div>
                    </div>
                    <Badge type={submission.track ? 'info' : 'submitted'}>{submission.track}</Badge>
                  </div>

                  <div className="space-y-3">
                    {RUBRIC.map(rubricItem => (
                      <div key={rubricItem.id} className="flex items-center justify-between rounded-xl border border-white/5 bg-white/[0.02] px-4 py-3">
                        <div>
                          <div className="text-[11px] font-bold text-text-1">{rubricItem.label}</div>
                          <div className="text-[9px] uppercase tracking-widest text-text-3">Weight {rubricItem.weight}%</div>
                        </div>
                        <div className="text-right">
                          <div className="text-sm font-mono font-bold text-white">{submittedEvaluation.scores?.[rubricItem.id] ?? scores[rubricItem.id] ?? 0}</div>
                          <div className="text-[9px] uppercase tracking-widest text-text-3">/100</div>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  {note && (
                    <div className="mt-4 rounded-xl border border-white/5 bg-black/10 px-4 py-3">
                      <div className="text-[10px] text-text-3 font-bold uppercase tracking-widest mb-2">Your note</div>
              ...
              <div className="space-y-4">
                <div className="rounded-[22px] border border-white/5 bg-gradient-to-b from-white/[0.03] to-transparent p-6">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="rounded-[18px] border border-green/15 bg-green/5 p-4 text-center">
                      <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-green mb-3">Evaluator Score</div>
                      <div className="mx-auto mb-3 flex w-fit items-center justify-center rounded-full border border-green/15 bg-bg-2/80 p-3">
                        <div className="text-4xl font-display font-black text-white">{submittedEvaluation.weightedScore ?? weightedScore}</div>
                      </div>
                      <div className="text-[11px] text-text-3">Weighted final after rubric scoring</div>
                    </div>

                    <div className="rounded-[18px] border border-purple/15 bg-purple/5 p-4 text-center">
                      <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-purple mb-3">AI Suggestion</div>
                      <div className="mx-auto mb-3 flex w-fit items-center justify-center rounded-full border border-purple/15 bg-bg-2/80 p-3">
                        <div className="text-4xl font-display font-black text-white">{aiScore}</div>
                      </div>
                      <div className="text-[11px] text-text-3">Pre-score benchmark estimate</div>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 rounded-[18px] border border-white/5 bg-bg-2/50 px-5 py-4">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <div className="text-[10px] text-text-3 font-bold uppercase tracking-[0.18em] mb-1">Delta</div>
                        <div className="text-sm text-text-2">Difference between your score and the AI recommendation</div>
                      </div>
                      <div className={`text-2xl font-display font-black ${scoreDelta >= 0 ? 'text-green' : 'text-red'}`}>
                ...
                <div className="rounded-[22px] border border-white/5 bg-bg-2/40 p-5">
                  <div className="text-[10px] text-text-3 font-bold uppercase tracking-[0.18em] mb-3">Next Step</div>
                  <p className="text-sm leading-relaxed text-text-2 mb-5">
                    This evaluation has been submitted successfully. You can return to your queue for the next assignment or stay here briefly to review the outcome.
                  </p>
