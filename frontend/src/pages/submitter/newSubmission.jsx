...
export default function NewSubmission() {
  ...
  const validate = () => {
    ...
    if (!form.projectUrl.trim() && !form.githubUrl.trim() && !form.demoUrl.trim() && !form.fileUrl.trim()) {
      e.access = 'Provide at least one project access option: project URL, GitHub URL, demo URL, or file URL.'
    }

    if (form.projectUrl && !isValidUrl(form.projectUrl)) e.projectUrl = 'Enter a valid URL'
    if (form.githubUrl && !isValidUrl(form.githubUrl)) e.githubUrl = 'Enter a valid URL'
    if (form.demoUrl && !isValidUrl(form.demoUrl)) e.demoUrl = 'Enter a valid URL'
    if (form.fileUrl && !isValidUrl(form.fileUrl)) e.fileUrl = 'Enter a valid URL'
  ...
  const handle = async () => {
    ...
    try {
      ...
      setErrors({
        submit: e.response?.data?.message ?? e.message ?? 'Submission failed. Please try again.',
      })
    } finally {
      setSaving(false)
    }
    ...
    <div className="fade-up max-w-3xl mx-auto">
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-[1.5fr_1fr] gap-5">
        <Card>
          <div className="flex flex-col gap-4">
            ...
            <div className="pt-2 border-t border-white/10">
              <div className="text-sm font-medium mb-3">Project Access for Evaluators</div>
              ...
              <div className="mt-4">
                <Input
                  ...
                  placeholder="e.g. Project Deck.pdf"
                />
              </div>

              {errors.access && <div className="mt-2"><ErrorMsg>{errors.access}</ErrorMsg></div>}
              {errors.projectUrl && <div className="mt-2"><ErrorMsg>{errors.projectUrl}</ErrorMsg></div>}
              {errors.githubUrl && <div className="mt-2"><ErrorMsg>{errors.githubUrl}</ErrorMsg></div>}
              {errors.demoUrl && <div className="mt-2"><ErrorMsg>{errors.demoUrl}</ErrorMsg></div>}
              {errors.fileUrl && <div className="mt-2"><ErrorMsg>{errors.fileUrl}</ErrorMsg></div>}
            </div>
        ...
        <div className="flex flex-col gap-3.5">
          ...
          <Card>
            <div className="text-[11px] uppercase tracking-widest mb-3 font-semibold text-text-2">
              Evaluator access checklist
            </div>
            <div className="text-[12px] text-text-3 space-y-2">
              <p>Provide at least one working link evaluators can open.</p>
              <p>Best options: live project URL, GitHub repository, demo URL, or presentation/file link.</p>
              <p>Broken or missing access links can delay scoring.</p>
            </div>
          </Card>
        </div>
      </div>
